...
void custom_crc_fold_init(deflate_state *s)
{
    s->strm->adler = 0;
}
...
TEST(crc_reset, 001)
{
    ...
    crc_fold_init_fake.custom_fake = &custom_crc_fold_init;
...
#ifdef X86_PCLMULQDQ_CRC
    ASSERT_EQ(1, x86_check_features_fake.call_count);
    ASSERT_EQ(1, crc_fold_init_fake.call_count);
#endif
}
